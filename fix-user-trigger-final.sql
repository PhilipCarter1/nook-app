-- =====================================================
-- FIX USER TRIGGER ISSUE (FINAL VERSION)
-- =====================================================
-- This script properly fixes the user trigger by handling all required columns
...
-- 5. CREATE A NEW, CORRECT TRIGGER FUNCTION THAT HANDLES ALL REQUIRED COLUMNS
CREATE OR REPLACE FUNCTION handle_new_user()
RETURNS TRIGGER AS $$
BEGIN
  -- Only create a record in public.users if the table exists
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
    WHERE table_schema = 'public' AND table_name = 'users'
  ) THEN
    -- Insert with all required columns, using metadata from auth.users
    ...
    VALUES (
      ...
      COALESCE((NEW.raw_user_meta_data->>'role')::text, 'tenant'),
      NOW(),
      NOW()
    )
...
SELECT 
  ...
  'User trigger has been updated to handle all required columns' as status;
...
SELECT 
  ...
  CASE 
    ...
    ) THEN '✅ TRIGGER WORKING - User created in public.users with all required columns'
    ELSE '❌ TRIGGER NOT WORKING - Check for errors above'
  END as status;

-- 10. SHOW WHAT WAS CREATED
SELECT 
  'CREATED USER DETAILS' as check_type,
  id,
  email,
  name,
  role,
...
SELECT 
  ...
  'The user trigger issue has been completely fixed. You can now run the test scenario script!' as status;

-- 13. SHOW WHAT TO DO NEXT
SELECT 
